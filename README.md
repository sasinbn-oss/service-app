...
7. **แดชบอร์ดเครื่องที่ดับอยู่** — ดูเครื่องสถานะ OFF แยกตามภาค เรียงตามความเร่งด่วน กรองตาม COCO/DODO และ Grade
...
### แดชบอร์ดเครื่องที่ดับอยู่

เมนู **เครื่องที่ดับอยู่** ในแท็บหน้าหลัก แสดงเครื่องสถานะ `OFF` แยกตามภาค
เรียงจากดับนานสุดก่อน เพราะนั่นคือลำดับที่ควรเข้าไปซ่อม

- การ์ดสรุป: OFF ทั้งหมด / COCO / DODO / ดับเกิน 72 ชม.
- กรองตาม COCO-DODO, Grade, และ "เฉพาะดับนาน"
- ค้นหาจากรหัสสาขา ชื่อสาขา หมายเลขเครื่อง หรือ zone
- ยุบ/กางรายภาคได้ พร้อมป้ายบอกจำนวนเครื่องและจำนวนที่ดับนาน
- **จอกว้าง** แสดงเป็นตารางเรียงลำดับได้จากหัวคอลัมน์
  **จอมือถือ** เปลี่ยนเป็นการ์ด เพราะตาราง 7 คอลัมน์อ่านบนมือถือไม่ไหว

เกณฑ์ "ดับนาน" ตั้งไว้ที่ 72 ชม. ปรับได้ที่ `DEFAULT_STALE_HOURS` ใน `backend/src/routes/machines.ts`
หรือส่ง `?staleHours=` มากับคำขอ

**⚠ สถานะเครื่องยังไม่ได้ต่อกับระบบเก็บเงินจริง** — ตอนนี้เป็นข้อมูลตัวอย่างที่ใส่ด้วย

```bash
cd backend && npm run seed:machines
```

เมื่อพร้อมต่อของจริง ให้ยิงเข้า `POST /api/machines/sync` (ADMIN) เป็นชุด ไม่ต้องแก้หน้าจอ

```json
{ "machines": [
  { "branchCode": "CO0101", "machineCode": "W3", "status": "OFF", "lastTxnAt": "2026-08-13T09:12:00+07:00" }
] }
```

สาขาที่ยังไม่มีในระบบจะถูกข้ามและรายงานกลับมาใน `unknownBranches` ส่วนเครื่องที่ยังไม่มีจะถูกสร้างให้อัตโนมัติ
...
**เครื่องซัก/อบ**
- `GET /api/machines?status=OFF&ownership=&grade=&region=&search=&staleOnly=&staleHours=`
  — คืน `{ now, staleHours, summary, rows }` (กรองที่เซิร์ฟเวอร์ทั้งหมด)
- `GET /api/machines/filters` — รายชื่อภาคและ zone ที่มีอยู่จริง
- `POST /api/machines/sync` (ADMIN) — อัปเดตสถานะเป็นชุดจากระบบเก็บเงิน
...
|---|---|---|
| **หน้าหลัก** | งานหน้างานทั้งหมด — เครื่องที่ดับอยู่, เอกสารขอโอนสินค้า, ผู้ช่วย AI, วินิจฉัย, อะไหล่, รายงานตัว, บันทึกงาน, ใช้รถ, เบิกของ | ทุกคน |
...
| แดชบอร์ดเครื่องดับ | ดูได้ | ดูได้ + sync ข้อมูล |
